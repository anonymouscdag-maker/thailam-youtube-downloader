...
RUN mv /usr/local/bin/yt-dlp /usr/local/bin/yt-dlp-real \
    && printf '%s\n' \
      ...
      '  exec /usr/local/bin/yt-dlp-real --cookies /etc/secrets/youtube-cookies.txt --extractor-args "youtube:player_client=mweb" "$@"' \
      'else' \
      '  exec /usr/local/bin/yt-dlp-real --extractor-args "youtube:player_client=mweb" "$@"' \
      'fi' \
